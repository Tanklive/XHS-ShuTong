...
SELECTORS = {
    ...
    # Publish button (supports both legacy CSS and Web Component)
    "publish_button": "xhs-publish-btn, .publish-page-publish-btn button.bg-red",
...
class XiaohongshuPublisher:
        ...
        return self._evaluate(f"""
            (() => {{
                ...
                const hasRedBg = (node) => {{
                    ...
                    return m && parseInt(m[1]) > 200 && parseInt(m[2]) < 120 && parseInt(m[3]) < 120;
                }};

                // 1. Try exact selector (legacy)
                const button = document.querySelector(buttonSelector);
                if (visible(button)) {{
                    return toRect(button);
                }}

                // 2. Text-based search on ALL elements (not just buttons)
                const keywords = [
                    {json.dumps(SELECTORS["publish_button_text"])},
                    {json.dumps(SELECTORS["schedule_publish_button_text"])},
                ];
                for (const node of document.querySelectorAll("button, [role='button'], .d-button, div, span, a")) {{
                    if (!visible(node)) continue;
                    const ownText = [...node.childNodes]
                        .filter(c => c.nodeType === 3)
                        .map(c => c.textContent).join("").trim();
                    if (keywords.includes(ownText) && node.getBoundingClientRect().width >= 40) {{
                        return toRect(node);
                    }}
                }}

                // 3. Red-bg element with "发布" text (exact match, excludes "发布笔记")
                const pubText = {json.dumps(SELECTORS["publish_button_text"])};
                for (const node of document.querySelectorAll("div, span, button")) {{
                    if (!visible(node)) continue;
                    const ownText = [...node.childNodes]
                        .filter(c => c.nodeType === 3)
                        .map(c => c.textContent).join("").trim();
                    if (ownText === pubText && hasRedBg(node)) {{
                        return toRect(node);
                    }}
                }}

                // 4. Last resort: any visible red-bg element in publish page area (x>200 excludes sidebar)
                for (const node of document.querySelectorAll("div, span, button")) {{
                    if (!visible(node)) continue;
                    const r = node.getBoundingClientRect();
                    if (r.width >= 40 && r.height >= 20 && r.x > 200 && hasRedBg(node)) {{
    ...
    def _is_publish_button_ready(self) -> bool:
        ...
        ready = self._evaluate(f"""
            (() => {{
                ...
                const notDisabled = (node) => {{
                    if (node.hasAttribute("disabled")) return false;
                    if (node.getAttribute("submit-disabled") === "true") return false;
                    if (String(node.className || "").includes("disabled")) return false;
                    if (node.getAttribute("aria-disabled") === "true") return false;
                    return true;
                }};

                // 1. Try CSS selectors (legacy)
                const selectors = [
                ...
                for (const selector of selectors) {{
                    ...
                    if (visible(btn) && notDisabled(btn)) return true;
                }}

                // 2. Text-based search on ALL elements
                const keywords = [
                    {json.dumps(SELECTORS["publish_button_text"])},
                    {json.dumps(SELECTORS["schedule_publish_button_text"])},
                ];
                for (const node of document.querySelectorAll("button, [role='button'], .d-button, div, span, a")) {{
                    if (!visible(node) || !notDisabled(node)) continue;
                    const ownText = [...node.childNodes]
                        .filter(c => c.nodeType === 3)
                        .map(c => c.textContent).join("").trim();
                    if (keywords.includes(ownText) && node.getBoundingClientRect().width >= 40) {{
                ...
                // 3. Red-bg element with "发布" text
                const pubText = {json.dumps(SELECTORS["publish_button_text"])};
                for (const node of document.querySelectorAll("div, span, button")) {{
                    if (!visible(node) || !notDisabled(node)) continue;
                    const bg = getComputedStyle(node).backgroundColor;
                    const m = bg.match(/rgb\\((\\d+),\\s*(\\d+),\\s*(\\d+)\\)/);
                    if (!(m && parseInt(m[1]) > 200 && parseInt(m[2]) < 120 && parseInt(m[3]) < 120)) continue;
                    const ownText = [...node.childNodes]
                        .filter(c => c.nodeType === 3)
                        .map(c => c.textContent).join("").trim();
                    if (ownText === pubText) return true;
                }}
    ...
    def _upload_images(self, image_paths: list[str]):
        ...
        for index, file_path in enumerate(prepared_paths, start=1):
            ...
            # Dispatch change event to trigger page's upload handler
            self._send("Runtime.evaluate", {
                "expression": "document.querySelectorAll('input[type=\"file\"]').forEach(function(inp) { inp.dispatchEvent(new Event('change', {bubbles: true})); }); 'ok'",
            })
    ...
    def _click_publish(self, scheduled: bool = False):
        ...
        XHS uses a Vue 3 Web Component (<xhs-publish-btn>) for the publish button.
        CDP mouse events and JS dispatchEvent do NOT trigger Vue's internal handler.
        We try the preferred method first: calling the Vue component's _onPublish() directly.
        If that fails, fall back to CDP mouse click.
        """
        print("[cdp_publish] Clicking publish button...")
        self._sleep(ACTION_INTERVAL, minimum_seconds=0.25)
        self._wait_for_publish_button_ready(timeout_seconds=20.0)

        # --- Method 1: Call Vue component's _onPublish() directly (preferred) ---
        print("[cdp_publish] Trying Vue _onPublish() handler...")
        vue_result = self._evaluate("""
            (async function() {
                var btn = document.querySelector('xhs-publish-btn');
                if (!btn) return JSON.stringify({method: 'none', error: 'no xhs-publish-btn element'});
                if (typeof btn._onPublish !== 'function') {
                    return JSON.stringify({method: 'none', error: '_onPublish not a function', keys: Object.keys(btn).filter(function(k) { return k.startsWith('_on'); })});
                }
                try {
                    await btn._onPublish();
                    return JSON.stringify({method: 'vue_onPublish', success: true, url: window.location.href});
                } catch(e) {
                    return JSON.stringify({method: 'vue_onPublish', error: e.message});
                }
            })();
        """)
        try:
            vr = json.loads(vue_result)
            print(f"[cdp_publish] Vue handler result: {vr.get('method')} - {vr.get('success', vr.get('error', 'unknown'))}")
            if vr.get('success'):
                ...
                note_link = self._evaluate("""
                    (function() {
                        ...
                        if (noteId) return 'https://www.xiaohongshu.com/explore/' + noteId[0];
                        return null;
                    })();
                """)
                return note_link
        except Exception as e:
            print(f"[cdp_publish] Vue handler parse error: {e}, falling back to CDP mouse click")

        # --- Method 2: Fall back to CDP mouse events ---
        print("[cdp_publish] Falling back to CDP mouse click...")
